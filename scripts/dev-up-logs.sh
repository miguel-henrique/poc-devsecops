...
presentation_research_bridge() {
	_c "$_C_BLUE$_C_BOLD" "  📚  Enquadramento acadêmico (alinhado à monografia)"
	...
	_c "$_C_DIM" "       na PoC AWS completa essas camadas complementam a esteira (ver docs/)."
	echo ""
	...
	_c "$_C_DIM" "     │ ALB / Ingress / estático │ nginx: entrada HTTP, /api/ → pool de APIs    │"
...
presentation_post_apply_summary() {
	...
	echo ""
	_c "$_C_BOLD" "  Rede"
	_c "$_C_DIM" "    Recurso Terraform: docker_network → nome ${full_net}"
	...
	echo ""
	_c "$_C_BOLD" "  Dados"
	_c "$_C_DIM" "    Container: ${project}-db  →  analogia Amazon RDS (PostgreSQL), dados em volume."
	...
	for ((i = 0; i < replicas; i++)); do
		_c "$_C_DIM" "    Container: ${project}-api-${i}  →  analogia pods/tasks no EKS ou serviços ECS."
	done
	_c "$_C_DIM" "    Conceito acadêmico: disponibilidade horizontal; cada réplica atende /health e /api/."
	echo ""
	_c "$_C_BOLD" "  Borda / apresentação"
	_c "$_C_DIM" "    Container: ${project}-web  →  analogia ALB + Ingress + origem estática (S3/CloudFront simpl.)."
...
presentation_footer() {
	...
	echo ""
	_c "$_C_CYAN$_C_BOLD" "══════════════════════════════════════════════════════════════════════"
	_c "$_C_CYAN$_C_BOLD" "  📋  Inspeção manual (comandos para slides ou terminal gravado)"
	_c "$_C_CYAN$_C_BOLD" "══════════════════════════════════════════════════════════════════════"
	echo ""
